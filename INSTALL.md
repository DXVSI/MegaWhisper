...
Bundle удобен для локальной онлайн-установки приложения даже на чистой системе: в нем есть `RuntimeRepo` для загрузки KDE runtime 6.11 из Flathub. Пока Flathub предоставляет Qt 6.11.1, Flatpak содержит полный app-local Qt Multimedia с точными upstream-исправлениями QTBUG-147011 и core-hook race `4ee738562`. Перед release проверяются путь загруженной библиотеки, GStreamer plugin, active input/output lifecycle через production PulseAudio compatibility backend и 25 отдельных запусков через временно разрешенный direct PipeWire без version-based skip или retry masking:

```fish
flatpak install -y --user ./MegaWhisper-2.0.1-x86_64.flatpak
```
...
./MegaWhisper-2.0.1-x86_64.AppImage --appimage-updateinformation
env QT_QPA_PLATFORM=offscreen APPIMAGE_EXTRACT_AND_RUN=1 ./MegaWhisper-2.0.1-x86_64.AppImage --smoke-test
...
```fish
./MegaWhisper-2.0.1-x86_64.AppImage --remove-desktop-integration
```
...
```fish
env APPIMAGE_EXTRACT_AND_RUN=1 ./MegaWhisper-2.0.1-x86_64.AppImage
```
...
Только после успешной сверки импортируйте ключ и проверьте подписанный список хешей. Он содержит все девять payload assets, включая сам release key; `SHA256SUMS` и `SHA256SUMS.asc` не включаются в список из-за циклической зависимости:
...
Индивидуальные `.asc` рядом с каждым файлом начиная с `v2.0.1` не публикуются. Полнота и целостность top-level assets подтверждаются одной подписью `SHA256SUMS.asc`.

`MegaWhisper-VERSION-compliance.tar.zst` содержит SPDX SBOM, build provenance, notices, соответствующие исходники AppImage runtime, использованные source RPM, официальный Qt Multimedia source tarball и оба точных upstream patch. `MegaWhisper-VERSION-recovery.tar.zst` содержит подписанные candidate и rollback состояния Flatpak repository и Pages. Оба архива имеют внутренние inventories с именами, размерами и SHA-256.

`MegaWhisper-VERSION-source.tar.zst` создается из того же очищенного orphan snapshot, на который указывает публичный tag. Snapshot содержит expanded `whisper.cpp`, `SOURCE-GRAPH.txt`, оба Qt Multimedia patch и не содержит приватную Git history или внутренние файлы.
...
```fish
set stage (mktemp -d)
scripts/ci/build-native.sh 2.0.1 1 native $stage
